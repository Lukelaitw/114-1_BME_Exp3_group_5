...
def majority_voting(predictions, window_size=20):
    """
    對預測結果進行多數決投票
    
    Args:
        predictions: 原始預測結果陣列
        window_size: 滑動窗口大小
    ...
    if len(predictions) < window_size:
        # 如果預測數量少於窗口大小，直接返回原始預測
        return predictions
    
    voted_predictions = []
    
    # 處理前 window_size-1 個預測（使用可用的歷史數據）
    for i in range(window_size - 1):
        # 使用從開始到當前位置的所有預測進行投票
        window_predictions = predictions[:i+1]
        # 計算多數決
        unique, counts = np.unique(window_predictions, return_counts=True)
    ...
    for i in range(window_size - 1, len(predictions)):
        # 獲取滑動窗口內的預測
        window_predictions = predictions[i - window_size + 1:i + 1]
        # 計算多數決
        unique, counts = np.unique(window_predictions, return_counts=True)
        majority_vote = unique[np.argmax(counts)]
        voted_predictions.append(majority_vote)
...
def apply_majority_voting_to_predictions(y_pred, window_size=20):
    """
    對預測結果應用多數決投票
    ...
    Returns:
        經過多數決投票處理的預測結果
    """
    return majority_voting(y_pred, window_size)
#==========student postprocessing==========
...
def leave_one_subject_out_validation(use_majority_voting=True, voting_window_size=18):
...
#==========student postprocessing==========
        ...
        if use_majority_voting:
            y_pred_voted = apply_majority_voting_to_predictions(y_pred, voting_window_size)
            voting_accuracy = accuracy_score(y_test, y_pred_voted)
            final_accuracy = voting_accuracy
            final_predictions = y_pred_voted
        else:
            final_accuracy = original_accuracy
            final_predictions = y_pred
        cm = confusion_matrix(y_test, final_predictions, labels=[0, 1])
        # Store results
        results['accuracies'].append(final_accuracy)
        results['confusion_matrices'].append(cm)
        results['loss_curves'].append(classifier.get_loss_curve())
        results['subject_names'].append(test_subject)
        
        print(f"{test_subject}: Accuracy = {final_accuracy:.3f}")
#==========student postprocessing========== 
    return results
...
def plot_results(results):
    ...
    fig.suptitle('BCI Classifier (Optimized) - LOSO Cross-Validation Results', fontsize=16)
    ...
    plt.tight_layout()
    plt.savefig('bci_results_voting_data.png', dpi=300, bbox_inches='tight')
    plt.show()
...
def main():
   ...
   #     Config.N_FEATURES_SELECT = i
   #     print(f"N_FEATURES_SELECT: {Config.N_FEATURES_SELECT}")
#==========student postprocessing==========
    results = leave_one_subject_out_validation(use_majority_voting=True, voting_window_size=20)
#==========student postprocessing==========
